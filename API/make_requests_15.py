...
def requests_key(influx):       
    print('STARTING KEY\n')
    for val in Query.get_key_period(15): 
        # Make request
        try:
...
def requests_http(influx):  
    print('STARTING HTTP\n')
    for val in Query.get_http_period(15):  
        try:
...
def requests_token(influx):
    ...
    for val in Query.get_token_period(15):
...
def main():
    ...
    # add jobs
    scheduler.add_job(requests_basic, trigger="interval", args=[influx], minutes=15, id="15minjob_basic", next_run_time=datetime.now())
    scheduler.add_job(requests_key, trigger="interval", args=[influx], minutes=15, id="15minjob_key", next_run_time=datetime.now())
    scheduler.add_job(requests_http, trigger="interval", args=[influx], minutes=15, id="15minjob_http", next_run_time=datetime.now())
    scheduler.add_job(requests_token, trigger="interval", args=[influx], minutes=15, id="15minjob_token", next_run_time=datetime.now())
    # start the scheduler
    scheduler.start()
    try:
        while True:
