...
def requests_key(influx):       
    print('STARTING KEY\n')
    for val in Query.get_key_period(30): 
        # Make request
        try:
...
def requests_http(influx):  
    print('STARTING HTTP\n')
    for val in Query.get_http_period(30):  
        try:
...
def requests_token(influx):
    ...
    for val in Query.get_token_period(30):
...
def main():
    ...
    # add jobs
    scheduler.add_job(requests_basic, trigger="interval", args=[influx], minutes=30, id="30minjob_basic", next_run_time=datetime.now())
    scheduler.add_job(requests_key, trigger="interval", args=[influx], minutes=30, id="30minjob_key", next_run_time=datetime.now())
    scheduler.add_job(requests_http, trigger="interval", args=[influx], minutes=30, id="30minjob_http", next_run_time=datetime.now())
    scheduler.add_job(requests_token, trigger="interval", args=[influx], minutes=30, id="30minjob_token", next_run_time=datetime.now())
    # start the scheduler
    scheduler.start()
    try:
        while True:
        ...
        scheduler.remove_job('60minjob_key')
        scheduler.remove_job('60minjob_http')
        scheduler.remove_job('60minjob_token')
        scheduler.shutdown()
        scheduler.shutdown()
