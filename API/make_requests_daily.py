...
def main():
    ...
    # add jobs
    scheduler.add_job(requests_basic, trigger="interval", args=[influx], minutes=1440, id="dailyjob_basic", next_run_time=datetime.now())
    scheduler.add_job(requests_key, trigger="interval", args=[influx], minutes=1440, id="dailyjob_key", next_run_time=datetime.now())
    scheduler.add_job(requests_http, trigger="interval", args=[influx], minutes=1440, id="dailyjob_http", next_run_time=datetime.now())
    scheduler.add_job(requests_token, trigger="interval", args=[influx], minutes=1440, id="dailyjob_token", next_run_time=datetime.now())
    # start the scheduler
    scheduler.start()
    try:
        while True:
